...
| 2026-09-06 (evening) | Nothing varies -- a diagnostic. Run 37's entries (published v4, 381 legs / 2022-01-05..2026-01-28), split by whether the SIGNAL day (the last session strictly before `entry_date`) closed at the IDX auto-reject ceiling | 1 | **Position-level median and mean return, ARA signal day vs not, plus the share of entries in each bucket.** Legs are combined share-weighted into positions first -- TP1 partials are winners by construction and counting legs is the error that once inflated a published win rate from 26.3% to 49.3% | Motivated by a live-record finding today: signals close at ARA at 2.31% (5 of 216) against a 0.648% liquid-universe base rate, 3.6x, Poisson p ~ 0.014. Five events is too thin to act on, and none of the five was ever bought, so the live record cannot say what it would have cost. **I do not have a confident prediction, and the two obvious ones point opposite ways.** Buying the session after a limit-up move should be bad: the next open gapped +7.6%, +8.7%, -14.9%, -14.9% and +0.5% on those five, which is a lottery rather than a fill. But this strategy's whole edge is in the right tail, and a stock at ARA is by definition already in a large run -- so ARA entries could equally be where the tail lives. If the two buckets are indistinguishable, an ARA filter is not worth unfreezing anything for, and that is a useful answer too. The ARA bands are not assumed: they were derived from our own EOD history (35% below Rp200, 25% Rp200-5,000, 20% above), and they match the published IDX rule. |
| 2026-09-06 (night) | Nothing varies -- a diagnostic. Run 37's 168 stopped positions, asking WHEN their pre-stop peak occurred: inside the MIN_HOLD_DAYS=3 window (when TP1 and the trailing stop are both locked out but the SL is live) or after it | 1 | **Share of stopped positions whose highest close before the stop fell inside the first 3 trading days, and the median peak gain in that window.** Reported for all stopped positions, not a chosen subset | The council's peer review proposed widening the stop only after a position shows profit. Checking whether that is even coherent turned up something sharper. `cfg.MIN_HOLD_DAYS = 3` carries the comment "TP1/Trailing ditahan N hari bursa; SL selalu aktif", and `simulate_window()` gates TP1 and trailing on `hold_ok` while the SL branch is evaluated first and ungated. So the first three sessions are ASYMMETRIC: all downside protection live, all upside capture disabled. Exploratory numbers that led here, stated so they are not mistaken for the test: of 168 stopped positions only 5 were never in profit, 100 peaked above +5%, 47 above +10%, mean peak +7.75% -- but that used intraday highs over the whole hold, so it cannot say WHEN. Prediction: I expect a large share, but I do not have a number. If most peaks land inside the lockout window, the leak is the asymmetry, not the stop width, and the 3 ATR forward arm is aimed at the wrong thing. If they land after it, the min-hold gate is exonerated and the stop-width reading stands. |
