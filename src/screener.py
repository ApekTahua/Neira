...
# ----------------------------------------------------------------------
try:
    # Insert biasa - anon key cuma dapat izin INSERT/SELECT (RLS), bukan
    # DELETE/UPDATE. unique(run_date, stock_code) bikin re-run manual hari
    # yang sama gagal insert dgn aman (unique violation, ke-catch di bawah)
    # daripada nge-duplikat baris.
    rows = [
        {
