"""
backtest_v2d.py — V2d: v2c plus a 3-day signal-memory watchlist.

Adds one mechanism on top of v2c (ADTV liquidity floor kept, HMM as
exit-side risk management): a signal that clears get_signals() but loses
the slot/cash competition is no longer discarded outright. It joins a
persistent watchlist for up to cfg.WATCHLIST_MAX_DAYS trading days,
carrying its original (frozen, first-seen) confidence/ATR/TP/SL/buy-zone
snapshot, and competes again each subsequent day alongside fresh signals
for any slot that opens up.

Structural justification, not hindsight: capital/slot scarcity is a
portfolio constraint, not a market judgment on the setup's quality — a
good setup skipped only because max_positions was full shouldn't be
required to re-qualify from scratch. Explicitly NOT tuned by checking
which historical "missed" trades this would have caught; the grace
window (3 days) and invalidation rules were fixed before this file was
run, and are not adjusted based on this run's result.

A watchlist entry is dropped (not carried further) when: it executes, it
exceeds the grace window, today's close breaks below its frozen
buy_zone_low (the setup itself failed), or it's superseded by a fresh
signal on the same stock_code. Execution still uses the existing
gap-check against the entry's original signal_close — a watchlist entry
that has drifted too far since first sighting is naturally rejected by
that pre-existing guard, not by new bespoke logic.
...
BACKTEST_VERSION = os.environ.get("BACKTEST_VERSION", "v2d-dev")
BACKTEST_PUBLISH = os.environ.get("BACKTEST_PUBLISH", "false").lower() == "true"


def run_backtest_v2d():
    ...
    print("=" * 70)
    print("BACKTEST V2d - v2c + 3-day signal-memory watchlist")
    ...
    pending_entries = []
    last_sl_idx = {}
    watchlist = []  # persistent across days -- signal-memory grace window
    ...
    for day_idx, trade_date in enumerate(trading_days):
        ...
        if not signals.empty:
            for _, sig in signals.iterrows():
                buy_zone_low = sig.get("buy_zone_low")
                fresh_candidates[sig["stock_code"]] = {
                    "stock_code": sig["stock_code"],
                    ...
                    "trigger": sig["trigger"],
                    "buy_zone_low": float(buy_zone_low) if pd.notna(buy_zone_low) else None,
                }

        # Watchlist survivors: still eligible (not held, not superseded by a
        # fresh signal today, not expired, not price-invalidated).
        day_close_lookup = day_data.set_index("stock_code")["close_price"] if not day_data.empty else pd.Series(dtype=float)
        surviving_watchlist = []
        for w in watchlist:
            if w["stock_code"] in held_codes or w["stock_code"] in fresh_candidates:
                continue
            today_close = day_close_lookup.get(w["stock_code"])
            price_invalidated = (
                today_close is not None and w["buy_zone_low"] is not None and today_close < w["buy_zone_low"]
            )
            if w["days_on_watchlist"] >= cfg.WATCHLIST_MAX_DAYS or price_invalidated:
                continue
            w["days_on_watchlist"] += 1
            surviving_watchlist.append(w)

        combined = list(fresh_candidates.values()) + [{**w, "origin": "watchlist"} for w in surviving_watchlist]
        combined = sorted(combined, key=lambda c: c["confidence"], reverse=True)[:15]
        selected_codes = {c["stock_code"] for c in combined}
        pending_entries.extend(combined)

        # Fresh signals not selected this round join the watchlist fresh;
        # watchlist survivors that were selected this round are consumed
        # (used their shot, win or lose at execution).
        watchlist = [w for w in surviving_watchlist if w["stock_code"] not in selected_codes]
        for code, cand in fresh_candidates.items():
            if code not in selected_codes and code not in held_codes:
                watchlist.append({**cand, "days_on_watchlist": 0})
    ...
    for pos in positions:
        ...
        trades.append({
            ...
            "exit_reason": "END", "trigger": pos["trigger"], "origin": pos.get("origin", "fresh"),
            "hold_days": pos["hold_days"],
        })

    total_trades = len(trades)
    if total_trades == 0:
        print("\n[BACKTEST V2d] No trades executed in test window.")
        return
    ...
    print("\n" + "=" * 70)
    print("BACKTEST V2d RESULTS (out-of-sample test window only)")
    print("=" * 70)
    ...
    print("\n--- Breakdown by Origin (fresh signal vs watchlist grace window) ---")
    for origin in ["fresh", "watchlist"]:
        subset = df_trades[df_trades["origin"] == origin]
        if not subset.empty:
            o_win = (subset["pnl"] > 0).sum()
            o_total = len(subset)
            o_pnl = subset["pnl"].sum()
            print(f"  {origin:10s}: {o_total:3d} trades ({o_win:3d} win), total PnL Rp {o_pnl:>14,.0f}")
    ...
    notes = (
        f"V2d: same as v2c (ADTV>=Rp{cfg.ADTV_MIN:,.0f} liquidity floor at entry, HMM as "
        f"exit-side risk management only) plus a {cfg.WATCHLIST_MAX_DAYS}-day signal-memory "
        f"watchlist for signals that clear get_signals() but lose the slot/cash competition. "
