"""
backtest_v2b.py — V2b: HMM/ADTV as risk management, not entry gates.

Tests the hypothesis (raised, then contaminated by iterative result-peeking,
in the archived branch tag `archive/v2-tuned-pre-reset`): that hard-gating
entries on ADTV/HMM caps alpha because it excludes trades V1's own edge
depends on, and that ADTV/HMM are better used to manage risk on positions
already taken rather than to reject candidates outright.

Differences from backtest_v2.py:
  - get_signals() called ungated (no adtv_min/hmm_gate) — identical entry
    universe to V1's own screener/backtest.py. No new entry-side liquidity
    rule is added: the existing LIQ_CAP_PCT lot cap (volume-proportional,
    already in both V1 and V2) already shrinks position size for illiquid
    names organically, without a second, redundant, tunable knob.
  - A held position's own HMM state is checked every day and used purely
    for risk management: pre-TP1, an HMM state that reads BEARISH forces
    an immediate exit at today's close (SL still checked first — capital
    protection is never suppressed); post-TP1, a BEARISH read halves the
    trailing-stop width (cfg.HMM_BEARISH_RISK_CUT) instead of gating
    anything at entry.

This is a single, pre-registered variant: run once against the same test
window as backtest_v2.py, record the result, and do not iterate on these
parameters afterward based on what comes out — that would repeat the exact
mistake this file exists to correct for. If the parameters need revisiting,
...
BACKTEST_VERSION = os.environ.get("BACKTEST_VERSION", "v2b-dev")
BACKTEST_PUBLISH = os.environ.get("BACKTEST_PUBLISH", "false").lower() == "true"


def run_backtest_v2b():
    ...
    print("=" * 70)
    print("BACKTEST V2b - HMM/ADTV as risk management, not entry gates")
    ...
    for day_idx, trade_date in enumerate(trading_days):
        ...
        for pos in positions:
            ...
            if not pos["tp1_hit"]:
                if low_price <= pos["sl_price"]:
                    ...
                    # V2b: HMM turning bearish on a held, not-yet-TP1'd
        ...
        # ---- New signals: UNGATED, identical entry universe to V1 ----
    ...
    if total_trades == 0:
        print("\n[BACKTEST V2b] No trades executed in test window.")
        return
    ...
    print("\n" + "=" * 70)
    print("BACKTEST V2b RESULTS (out-of-sample test window only)")
    print("=" * 70)
    ...
    notes = (
        f"V2b: ungated entries (identical universe to V1), HMM state used as exit-side "
        f"risk management only (immediate exit on BEARISH pre-TP1, {cfg.HMM_BEARISH_RISK_CUT}x "
        f"trailing width post-TP1) instead of an entry gate. No ADTV entry filter — existing "
        f"volume-proportional LIQ_CAP_PCT lot cap already shrinks illiquid-name position size. "
