"""sweep_broker_divergence_gate.py -- SCRATCH/RESEARCH ONLY, walk-forward +
sensitivity sweep for V4_DIVERGENCE_GATE (research candidate,
docs/V3_FINDINGS_LOG.md 2026-08-25 entry, per-stock smart-money divergence).
Same structure as sweep_broker_flow_gate.py -- one subprocess per grid cell.

Axes:
  V4_DIVERGENCE_MIN         -- gate threshold on the N-day rolling divergence ratio
  V4_DIVERGENCE_WINDOW_DAYS -- the rolling window itself (sensitivity axis)

V4_ATR_PRICE_RATIO_MAX pinned to 0.08 throughout (current live config), same
reconfirmed baseline as the market-wide sweep: mean alpha +26.17%, mean PF
1.95, 7/9 beat-bench, 4/9 win>50%, 366 trades.

Usage:
    SUPABASE_URL=... SUPABASE_KEY=... python src/sweep_broker_divergence_gate.py
"""
...
THRESHOLDS = _axis("SWEEP_DIV_MIN", ["off", "-0.1", "-0.05", "0.0", "0.05", "0.1", "0.2"])
WINDOWS = _axis("SWEEP_DIV_WINDOW", ["5"])
...
def main():
    ...
    for thr, win in itertools.product(THRESHOLDS, WINDOWS):
        ...
        if is_off:
            env["V4_DIVERGENCE_GATE"] = "0"
        else:
            env["V4_DIVERGENCE_GATE"] = "1"
            env["V4_DIVERGENCE_MIN"] = thr
            env["V4_DIVERGENCE_WINDOW_DAYS"] = win
        proc = subprocess.run(
        ...
        wdf.to_csv(os.path.join("C:/Users/Apek/AppData/Local/Temp/claude/d--Neira-Neira/"
                                 ...
                                 f"divergence_full_{thr}_{win}.csv"), index=False)
    ...
    print("\n" + "=" * 110)
    print("BROKER-DIVERGENCE GATE SWEEP -- one row per (threshold x window) cell")
    print("=" * 110)
    print(res.to_string(index=False))
    res.to_csv("sweep_broker_divergence_gate_summary.csv", index=False)
    print("\n[OK] Saved sweep_broker_divergence_gate_summary.csv")
